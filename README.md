...
- **UI:** music is toggled only by the Music button (board clicks never touch it). Setup Position and Endgame Examples are collapsible panels in their own card to the right of the moves table; the examples live in `examples/endgame_examples.js`. The **vs Human** button switches modes without resetting the board, so it also works while setting up a position or on a loaded endgame example (one human plays both colors).
...
- **Tests:** `node --test` → 72 tests / 15 suites. Uses a sandboxed VM harness with DOM stubs (functional `classList`); `setBoard` resets setup/music/panel state.
...
| File | Lines | Purpose |
|------|-------|---------|
| `index.html` | 151 | Main HTML page with sidebar UI, canvas, moves panel + eval bar, a separate utility card (Setup Position / Endgame Examples) to the right of the moves panel, background-music audio, game-over modal |
| `style.css` | 709 | Dark-themed UI styling with gradient background |
...
| `main.js` | 220 | Wiring: all event listeners, canvas click handler, setup/examples panel toggles, `ENDGAME_EXAMPLES` loading, startup |
...
| `test/ui.test.js` | 222 | DOM-coupled UI: evaluation bar, music toggle, endgame-examples panel toggle, vs-human mode switching, move timing |
...
| `#setup-btn` | Toggles the Setup Position panel (revealed in the utility card to the right of the moves table): starts setup (`startPositionSetup`), or **Cancel Setup** closes it (`cancelPositionSetup`); opening it hides the Endgame Examples panel |
...
- **Setup Position** — a collapsible panel in the utility card to the right of the Move Table (its own column), toggled by the **Setup Position** button. Opens a palette-driven editor: place any of 7 piece types in red or black, erase, clear, load the standard start, pick which side moves first; **Start Game** validates (kings compulsory in-palace, not facing) and commits as a fresh game, **Cancel Setup** restores the previous position/history exactly. New Game, examples, and mode switches auto-exit setup mode. The **vs Human** button does NOT auto-exit setup — it just keeps the board in both-human mode, so a custom setup can be played by one person on both sides.
- **Endgame Examples** — a collapsible panel in the utility card to the right of the Move Table (its own column), toggled by the **Endgame Examples** button — exactly like Setup Position. Contains the six example positions in their own folder (`examples/`); opening it while in setup mode exits setup first. Loading an example keeps the current mode, so in vs-Human mode one person plays both sides of the puzzle; in vs-Computer mode the human plays Red and the AI takes Black.
...
- **Standard flow at the start of a session:** run `node --test` to confirm the baseline (currently 72 tests / 15 suites), read the README's **Current State** (top, fastest resume) + Architecture to reload context, and inspect `git status`/`git log --oneline` for where things were left off.
